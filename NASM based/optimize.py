...
def GenerateLocals(consolidate):
    if consolidate:
        ...
        temp_seq=[a.Address+a.Size for a in g.locallist.values() if a.Address!=None]
        if temp_seq!=[]:
            g.locals_end = max(temp_seq)
        else:
            g.locals_end=g.locals_begin
        ...
        if line.LocalVar:
            ...
            for j,symbol in enumerate(line.TokenList):
                if symbol.type==BUFF_LOCAL:
                    if line.LastLabel+"."+symbol.data in g.locallist:
                        if g.locallist[line.LastLabel+"."+symbol.data].Address==None:
                            ...
                            g.proglist[k].Comment=" ;"+symbol.data
                            calculate=True
                    else:
                        if g.locallist[symbol.data].Address==None:
                            calculate=False
                        else:
                            g.proglist[k].TokenList[j]=Token(g.locallist[symbol.data].Address,BUFF_NUM)
                            g.proglist[k].Comment=" ;"+symbol.data
                            calculate=True
...
#TODO: combine some of the below loops if possible
    ...
    for func in funcs_left:
        for local in g.callgraph[func].Locals:
            ...
            if address_found==False:
                ...
                #print(func+"."+local.Name+":"+str(potential_address+g.locals_begin))
                for byte in range(local.Size):
                    #print(potential_address+byte)
                    #mem_list[potential_address+byte].append(LocalInfo(local.Name,func,local.Size,byte))
                    mem_list.append([LocalInfo(local.Name, func, local.Size, byte)])
                    address_found=True
    ...
    try:
        #fptr=open(sys.path[0]+"\\debug\\template.html",mode="r")
    ...
    try:
        #fptr=open(sys.path[0]+"\\output\\callgraph.html",mode="w")
