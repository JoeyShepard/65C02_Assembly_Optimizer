...
IGNORE_LIST=['LOCAL_LABEL','CONSOLE_SHOW_GLOBALS','CONSOLE_SHOW_LOCALS','CONSOLE_SHOW_FUNCS','FUNC_NAME',
            'OPTIMIZER_BUILTIN','LOCALS_INDEX','LOCALS_COUNT','ASSIGN_LOCAL','ASSIGN_GLOBAL']
        ...
        if len(templist)>=2:
            if templist[0].type==BUFF_ALPHA:
                if templist[1].type==BUFF_EQUAL or templist[1].type==BUFF_SET:
                    if templist[0].data.upper()=='OPTIMIZER_GLOBAL_SIZE':
                        if templist[2].type!=BUFF_NUM:
                            print('Error: non-integer assignment to size of global variable. (Symbollic assignment not supported but can be added...):',g.proglist[i].RawStr)
                            g.errorhalt=True
                            return
                        else:
                            lastglobalsize=templist[2].data
                            g.proglist[i].Resolved=True
                            g.proglist[i].Ignore=True
                    elif templist[0].data.upper()=='OPTIMIZER_LOCAL_SIZE':
                        if templist[2].type!=BUFF_NUM:
                            print('Error: non-integer assignment to size of local variable. (Symbollic assignment not supported but can be added...):',g.proglist[i].RawStr)
                            g.errorhalt=True
                            return
                        else:
                            g.proglist[i].LocalSize=templist[2]
                            lastlocalsize=templist[2].data
                            g.proglist[i].Resolved=True
                            g.proglist[i].Ignore=True
                    elif templist[0].data.upper()=='OPTIMIZER_CONSOLIDATE_LOCALS':
                        if templist[2].type!=BUFF_BOOLEAN:
                            print('Error: non boolean assignment to OPTIMIZER_CONSOLIDATE_LOCALS.',g.proglist[i].RawStr)
                            g.errorhalt=True
                            return
                        else:
                            g.consolidate_locals=templist[2].data
                            g.proglist[i].Resolved=True
                            g.proglist[i].Ignore=True
                    elif templist[0].data.upper()=='LOCALS_BEGIN':
                        if templist[2].type!=BUFF_NUM:
                        ...
                        if templist[2].data.upper()=='ASSIGN_LOCAL':
                            g.locallist[parentlabel+"."+templist[0].data]=Localvar(templist[0].data,parentlabel,lastlocalsize,None)
                            g.proglist[i].Resolved=True
                            g.proglist[i].Ignore=True
                        elif templist[2].data.upper()=='ASSIGN_GLOBAL':
                            #Keep global as simple SET statement (less overhead)
                            g.proglist[i].TokenList[2]=Token(g.ZP_counter,BUFF_NUM)
                            g.ZP_counter+=lastglobalsize
            ...
            if line[j].type==BUFF_ALPHA or line[j].type==BUFF_LABEL:
                if line[j].data in g.builtin_dict:
                    ...
                    if g.evallist[line[j].data].Resolved:
                        ...
                        line=line[:j]+g.evallist[line[j].data].TokenList+line[j+1:]
                    else:
                        #print("      unresolved")
                        allresolved=False
                else:
...
def CalculateLine(line,lineobj):
    ...
    for i in line:
        ...
        BUFF_OPEN_BRACKET, BUFF_CLOSE_BRACKET, BUFF_PCSTAR, BUFF_ASMWORD, BUFF_FCB, BUFF_FDB, BUFF_BOOLEAN):
            objstack.append(i)
            #print("      Object:",i.data)
                    ...
                    if searchmode==MODE.IMPLICIT:
                        ...
                        if searchmode==MODE.ARG_UNKNOWN_X:
                            if line.TokenList[0].data=="JMP":
                                if line.FormatStr=="O(*,X)": searchmode=MODE.INDIRECT_ABS_X
                                else:
                                    print("Error: invalid instruction format:",line.RawStr.strip())
                                    sys.exit()
                            elif op_size==1:
